import {Image, View, Text, TouchableOpacity, ActivityIndicator, FlatList } from 'react-native'
...
const HomeScreen = () => {
  ...
  useEffect(() => {
    ...
    const FetchLoggedUserChats = async () => {
      ...
      const queryResult2 = query(chatRef, where('chatters', '>=', `${username}`))
      ...
      const unsubscribe2 = onSnapshot(queryResult, (querySnapshot)=>{
        setIsLoading(false);
        querySnapshot.forEach((doc)=>{
          if(doc.data().chatters.includes(username)){
            const chats = doc.data().chatters
